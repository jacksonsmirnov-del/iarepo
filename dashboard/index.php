    ...
    <?php else: ?>
      <div class="resource-list">
        <?php foreach ($collections as $c): ?>
          <div class="resource-item">
            <div class="resource-info">
              <h3>📁 <?= h($c['title']) ?> <span class="badge <?= $c['is_public'] ? 'badge-community' : 'badge-draft' ?>"><?= $c['is_public'] ? 'público' : 'privado' ?></span></h3>
              <div class="resource-meta"><span>📦 <?= (int)$c['item_count'] ?> recursos</span><span><?= date('d/m/Y', strtotime($c['created_at'])) ?></span></div>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
...
// New collection
document.getElementById('newCollBtn')?.addEventListener('click',async()=>{
  const title=prompt('Nombre de la colección:');
  if(!title) return;
  try{
    const res=await fetch('/api/collections.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title})});
